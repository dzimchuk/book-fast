...
{
    internal class ListBookingsByUserQuery : IQuery<BookFastContext, List<Booking>>
    {
        private readonly string user;

        public ListBookingsByUserQuery(string user)
        {
            this.user = user;
        }

        public async Task<List<Booking>> ExecuteAsync(BookFastContext model)
        {
            return await (from b in model.Bookings
